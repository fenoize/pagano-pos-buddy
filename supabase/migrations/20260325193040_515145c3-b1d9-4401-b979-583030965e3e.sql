-- 1. Update app_orders_kitchen view to include tracking/delivery fields
CREATE OR REPLACE VIEW public.app_orders_kitchen AS
SELECT id, order_number, status, created_at, updated_at, fulfillment, items, total, notes, nombre_resumen,
       delivery_lat, delivery_lng, customer_id, delivery_address, delivery_comuna, payment_method, payment_runas
FROM orders o;

-- 2. Update create_order_with_context to persist delivery_lat/delivery_lng
...
BEGIN
  ...
  VALUES (
    ...
    NULLIF((p_order_data->>'delivery_lat'), '')::double precision,
    NULLIF((p_order_data->>'delivery_lng'), '')::double precision
  )
...
-- 3. RPC for customer to read tracking data (bypasses RLS)
CREATE OR REPLACE FUNCTION public.get_delivery_tracking_for_order(p_order_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
...
DECLARE
  v_result jsonb;
BEGIN
  SELECT row_to_json(dt)::jsonb INTO v_result
