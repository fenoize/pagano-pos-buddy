ALTER TABLE public.online_order_settings
  ADD COLUMN IF NOT EXISTS mixed_payment_enabled boolean NOT NULL DEFAULT false;

CREATE OR REPLACE FUNCTION public.get_online_order_settings(p_user_id uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  SELECT jsonb_build_object(
    ...
    'mixed_payment_enabled', mixed_payment_enabled,
...
CREATE OR REPLACE FUNCTION public.update_online_order_settings(p_settings jsonb, p_user_id uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  ...
  IF v_record_id IS NULL THEN
    ...
    VALUES (
      ...
      COALESCE((p_settings->>'mixed_payment_enabled')::boolean, false)
    )
  ...
  ELSE
    ...
    SET
      ...
      mixed_payment_enabled = COALESCE((p_settings->>'mixed_payment_enabled')::boolean, mixed_payment_enabled),
      updated_at = now()
