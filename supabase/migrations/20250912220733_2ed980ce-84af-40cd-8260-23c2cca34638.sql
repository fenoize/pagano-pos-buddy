...
USING (
  EXISTS (
    SELECT 1 FROM users 
    WHERE id = auth.uid() 
    AND role IN ('Administrador', 'Cajero', 'Caja') 
    AND active = true
  )
);
...
USING (
  EXISTS (
    SELECT 1 FROM users 
    WHERE id = auth.uid() 
    AND role IN ('Administrador', 'Cajero', 'Caja', 'Cocina', 'Preparador') 
    AND active = true
  )
);
...
USING (
  EXISTS (
    SELECT 1 FROM users 
    WHERE id = auth.uid() 
    AND role IN ('Administrador', 'Cocina', 'Preparador') 
    AND active = true
  )
)
WITH CHECK (
...
USING (
  EXISTS (
    SELECT 1 FROM users 
    WHERE id = auth.uid() 
    AND role IN ('Administrador', 'Cajero', 'Caja', 'Cocina', 'Preparador', 'Reparto') 
    AND active = true
  )
);
...
USING (
  EXISTS (
    SELECT 1 FROM users 
    WHERE id = auth.uid() 
    AND role IN ('Administrador', 'Cajero', 'Caja', 'Cocina', 'Preparador', 'Reparto', 'Viewer') 
    AND active = true
  )
);
