...
DROP POLICY "Anyone can insert campaigns" ON public.loyalty_campaigns;
DROP POLICY "Anyone can update campaigns" ON public.loyalty_campaigns;
DROP POLICY "Anyone can delete campaigns" ON public.loyalty_campaigns;

-- Create RPC for campaign CRUD (admin-only via staff context check)
...
BEGIN
  -- Get staff context
  v_staff_id := NULLIF(current_setting('app.user_id', true), '')::uuid;
