-- Create simple hash function using built-in MD5 for salt generation
...
BEGIN
  ...
  RETURN '$2b$10$' || salt || substr(password_hash, 1, 31);
END;
$$;
...
DECLARE
  ...
  computed_hash text;
BEGIN
  -- Handle our custom format $2b$10$[22-char-salt][31-char-hash]
  IF hash LIKE '$2b$10$%' AND length(hash) >= 60 THEN
    ...
    computed_hash := substr(encode(digest(salt || password, 'sha256'), 'hex'), 1, 31);
...
BEGIN
  ...
  IF is_valid_password AND length(user_record.pass_hash) < 60 THEN
