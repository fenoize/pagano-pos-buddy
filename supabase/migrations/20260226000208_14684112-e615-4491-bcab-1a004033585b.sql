...
BEGIN
  ...
  LOOP
    v_item_id := COALESCE(
      (v_receipt->>'itemId')::uuid,
      (v_receipt->>'item_id')::uuid
    );
    v_qty_received := COALESCE(
      (v_receipt->>'qtyReceived')::numeric,
      (v_receipt->>'qty_received')::numeric
    );
    ...
    SET qty_received = qty_received + v_qty_received
    WHERE id = v_item_id;

    v_any_received := true;

    IF p_ingress_to_inventory THEN
      -- Map UOM by NAME (codes differ between new/legacy tables)
      v_stock_uom_id := NULL;
      
      SELECT u_new.name INTO v_uom_name 
      ...
      IF v_uom_name IS NOT NULL THEN
        ...
        WHERE lower(u_old.name) = lower(v_uom_name) 
        LIMIT 1;
      END IF;
      ...
      IF v_stock_uom_id IS NULL AND v_item.base_uom_id IS NOT NULL THEN
        SELECT u_new.name INTO v_uom_name 
        FROM units_of_measure u_new 
        WHERE u_new.id = v_item.base_uom_id;
      ...
      INSERT INTO stock_moves (
        warehouse_id, raw_material_id, move_type, qty,
        uom_id, unit_cost, note, related_purchase_id
      ) VALUES (
        v_order.warehouse_id, v_item.raw_material_id, 'in', v_qty_received,
        v_stock_uom_id, v_item.unit_cost,
        'Recepción OC ' || v_order.po_number, p_order_id
      );

      INSERT INTO stock_balances (warehouse_id, raw_material_id, qty)
      VALUES (v_order.warehouse_id, v_item.raw_material_id, v_qty_received)
