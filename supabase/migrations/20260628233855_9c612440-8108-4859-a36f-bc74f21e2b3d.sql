...
DECLARE
  cp_record RECORD;
  burger_slot_id uuid := NULL;
  burger_slot_product_id uuid := NULL;
...
BEGIN
  ...
  IF combo_variant_names IS NOT NULL AND array_length(combo_variant_names, 1) IS NOT NULL THEN
    SELECT ci.id, ci.default_product_id
      INTO burger_slot_id, burger_slot_product_id
    FROM combo_items ci
    WHERE ci.combo_product_id = cp_record.id
      AND ci.default_product_id IS NOT NULL
      AND EXISTS (
        SELECT 1
  ...
  FROM combo_items ci
  WHERE ci.combo_product_id = cp_record.id
    AND (burger_slot_id IS NULL OR ci.id <> burger_slot_id)
  ...
  LOOP
    burger_price := 0;
    IF burger_slot_product_id IS NOT NULL THEN
      SELECT pvo2.price INTO burger_price
      FROM product_variant_options pvo2
      JOIN category_variants cv2 ON cv2.id = pvo2.category_variant_id
      WHERE pvo2.product_id = burger_slot_product_id
