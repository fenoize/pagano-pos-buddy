-- =====================================================
-- MIGRACIÓN: Módulo de Cierres Financieros PRO
-- =====================================================

-- 1. Extender tabla financial_closures con nuevas columnas
ALTER TABLE public.financial_closures
ADD COLUMN IF NOT EXISTS total_cash numeric(12,2) DEFAULT 0,
ADD COLUMN IF NOT EXISTS total_pos numeric(12,2) DEFAULT 0,
ADD COLUMN IF NOT EXISTS total_transfer numeric(12,2) DEFAULT 0,
ADD COLUMN IF NOT EXISTS total_app numeric(12,2) DEFAULT 0,
...
CREATE INDEX IF NOT EXISTS idx_financial_closures_dates 
  ON public.financial_closures(date_start, date_end);
  
CREATE INDEX IF NOT EXISTS idx_financial_closures_type 
  ON public.financial_closures(period_type);
  
CREATE INDEX IF NOT EXISTS idx_financial_closures_created_by 
  ON public.financial_closures(created_by);

-- 3. Crear función mejorada finance_generate_closure_v2
...
BEGIN
  ...
  s := (_start::text || ' 00:00:00')::timestamp AT TIME ZONE _tz;
  e := ((_end::date + interval '1 day')::text || ' 00:00:00')::timestamp AT TIME ZONE _tz;
...
GRANT EXECUTE ON FUNCTION public.finance_generate_closure_v2 TO authenticated;